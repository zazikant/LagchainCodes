function onEdit(e) {
    ...
    if (colName.toLowerCase() === 'email') {
      ...
      // Iterate through the data to find unique values based on email
      data.forEach(function(row) {
        var email = row[colIndex - 1]; // Assuming email is in the selected column
        var index = uniqueData.findIndex(function(item) {
          return item[colIndex - 1] === email;
        });
